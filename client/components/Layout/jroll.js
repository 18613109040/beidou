...
(function (window, document, Math) {
  ...
  JRoll = function (el, options) {
    ...
    try {
      ...
      me.wrapper.addEventListener('test', null, opts)
    } catch (e) {}

    function addEvent (type, method) {
      me.wrapper.addEventListener(type, method, supportsPassiveOption ? { passive: false } : false)
    }
    ...
    me.wrapper.addEventListener('resize', _resize)
    me.wrapper.addEventListener('orientationchange', _resize)
