...
class UploadService extends Service {
  ...
  async index(payload) {
    ...
    if (isPaging) {
      if (search) {
        if (kind) {
          res = await this.ctx.model.Attachment.find({ filename: { $regex: search }, extname: { $in: attachmentKind[`${kind}`] } }).skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 })
            .exec();
        } else {
          res = await this.ctx.model.Attachment.find({ filename: { $regex: search } }).skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 })
            .exec();
        }
        count = res.length;
      } else if (kind) {
        res = await this.ctx.model.Attachment.find({ extname: { $in: attachmentKind[`${kind}`] } }).skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 })
          .exec();
        count = await this.ctx.model.Attachment.count({ extname: { $in: attachmentKind[`${kind}`] } }).exec();
      } else {
        res = await this.ctx.model.Attachment.find({}).skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 })
          .exec();
